<script>
  ...
  function filterRows() {
    filteredEvents = allEvents.filter((row) => {
      ...
      const eventDateStr = row.event_timestamp
        ? new Date(row.event_timestamp * 1000).toISOString().split("T")[0]
        : "";
      const dateMatch =
        !searchParams.date ||
        (eventDateStr &&
          eventDateStr.includes(formatSearchedDate(searchParams.date)));
...
<div class="sub-header-container">
  <div class="sub-header">
    <img
      ...
      alt="Cross-drop graphic"
      class="subheader-image"
    />
    <h1>Cross-drop prevention</h1>
    <span>
      Midas Elbow prevents unloading errors by detecting the product before its
      dropped into the underground storage tank. View prevented contamination
      instances below, either live or by custom search fields.
    </span>
  </div>
              ...
              <div class="card-header">
                ...
                <div class="card-item">
                  ...
                  <span class="card-value" style="margin-left: 2vw;"
                    >Fuel: {row["Prevented Delivery "]}</span
                  >
                  <span class="card-value" style="margin-left: 2vw;"
                    >Zip: {row.Zip}</span
                  >

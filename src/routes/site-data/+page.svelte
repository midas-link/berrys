<script>
    ...
    filteredEvents = allEvents.filter((row) => {
      ...
      const businessUnitMatch =
        ...
        (row.BusinessUnitName &&
          row.BusinessUnitName.toLowerCase().includes(
            searchParams.businessUnit.toLowerCase()
          ));
      ...
      const siteMatch =
        !searchParams.site ||
        (row.SiteCode &&
          row.SiteCode.toLowerCase().includes(searchParams.site.toLowerCase()));
      ...
      const fuelMatch =
        !searchParams.fuel ||
        (row.Delivered &&
          row.Delivered.toLowerCase().includes(
            searchParams.fuel.toLowerCase()
          ));
  ...
  function hasSearchInput() {
    return (
      searchParams.businessUnit.trim() !== "" ||
...
<div class="sub-header-container">
  <div class="sub-header">
    <img
      ...
      alt="Gas_station Graphic"
      class="subheader-image"
    />
    <h1>Site Data</h1>
    <span>
      Gather insight into your deliveries by site location. You can dive into
              ...
              <div class="card-header">
                <div class="card-item">
                  <span class="card-label">
                    ...
                    "Just now"
                      ? "Just now"
                      : `${formatDate(formatEpochToDisplay(row.event_timestamp))} `}</span
                  >
                </div>
                <div class="card-item">
                  <span class="card-value" style="margin-left: 2vw;"
                    >Fuel: {row.Delivered}</span
                  >
                  <span class="card-value" style="margin-left: 2vw;"
                    >Site: {row.SiteCode}</span
                  >
                  <span
                    class="card-value"

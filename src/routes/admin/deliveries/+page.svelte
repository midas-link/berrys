<script>
  ...
  function searchDelivery(searchedDelivery) {
    filteredDeliveries = [];
    if(searchedDelivery.length===0) 
      filtered= false;
    deliveries.forEach(element => {
      if(element.Address.toLowerCase().includes(searchedDelivery.toLowerCase()))
        {
        filteredDeliveries.push(element);
        filtered= true;
        }
    });
  ...
  async function handleLogout(){
    ...
    async function fetchDeliveries() {
        ...
        try {
            ...
            console.log('Admin deliveries loaded:', deliveries); 

        } catch (err) {
            console.error('Error fetching admin deliveries:', err);
            error = err.message || 'Failed to load deliveries. Please try again.';
        } finally {
            isLoading = false; 
        }
    }
  ...
  ></script>
    ...
    <div class="main-container">
        <div class="dashboard-header">
            ...
            <input type="text" placeholder="Search Deliveries" class="search-bar" on:keydown={(e) => {if(e.key==='Enter'){ searchDelivery(e.target.value)} }}> 
        </div>
    ...
    {#if isLoading}
    <tr>
        <td> Loading Deliveries</td>
    </tr>
    {:else if error}
    <tr>
        <td>{error}</td>
    </tr>
    {:else if (filtered ? filteredDeliveries : deliveries).length > 0}
    {#each (filtered ? filteredDeliveries : deliveries) as row, index}
        <tr>
            <td>{row.Address}</td>
            ...
            <td>{row.Zip}</td>
            <td>{row.Delivered}</td>
            <td>{formatEpochToDisplay(row[`Delivery Start`])}</td>
            <td>{formatEpochToDisplay(row[`Delivery End`])}</td>
            <td>{row.SiteCode}</td>
